...
CATEGORY_META = {
    ...
    "security": {"emoji": "🔒", "desc": "Identity, authentication, OIDC, supply chain security"},
    "cloudflare": {"emoji": "☁️", "desc": "Pages, Workers, DNS, deployments"},
    "ai": {"emoji": "🤖", "desc": "AI agent frameworks, comparisons, deep dives"},
    "infrastructure": {"emoji": "⚙️", "desc": "Core devops: build tools, container runtimes, CI/CD SDKs"},
    "compliance": {"emoji": "🛡️", "desc": "CIS benchmarks, Lynis, Anjungan — plus 30+ technology compliance standards"},
    "networking": {"emoji": "🌐", "desc": "VPN, tunnels, mesh networking, remote access"},
    "storage": {"emoji": "💾", "desc": "Database backup, object storage, container registry, SQLite tools"},
    "monitoring": {"emoji": "📊", "desc": "Server monitoring, Docker log viewers, observability"},
}
...
ARTICLE_DESCS = {
    ...
    "security/dex-oidc": "Dex OIDC — federated OpenID Connect identity provider for self-hosted apps (Zot, Forgejo, Beszel)",
    "security/dokploy-basic-auth": "Adding basic auth to Compose applications on Dokploy via Traefik middleware",
    "networking/chisel": "Chisel: fast TCP/UDP tunnel over HTTP, alternative to frp and ngrok — client-server architecture",
    "networking/frp": "Fast reverse proxy for exposing local servers behind NAT or firewalls: client-server architecture with multiple tunnel types",
    "networking/netbird-selfhosted": "Self-hosted Netbird WireGuard mesh VPN with SSO, groups, policies, TURN relay, and routing peers",
    "networking/netmaker-selfhosted": "Self-hosted Netmaker WireGuard mesh VPN platform: installation, ingress/egress gateways, ACL, and automation",
    "networking/warpgate": "Warpgate — smart SSH bastion host with OIDC auth, session recording, and web-based SSH client",
    "networking/wireguard": "WireGuard VPN: installation, hub & spoke topology, site-to-site, NAT traversal, advanced config, security, and migration from OpenVPN",
    "storage/database-backup": "Database backup guide for PostgreSQL, MySQL, and MariaDB: dedicated users, strategies, automation scripts, and test restore",
    "storage/database-backup-incremental": "Incremental database backup guide: pgBackRest, WAL archiving, XtraBackup, binary logs, MariaDB Backup — with tool comparison and pitfalls",
    "storage/kopia-backup": "Kopia backup with Cloudflare R2: incremental, dedup, encrypted backup for VPS",
    "storage/object-storage-comparison": "Comparison of S3-compatible object storage providers: R2, S3, GCS, Backblaze B2",
    "storage/zot-registry": "Lightweight OCI-compliant container registry with Zot: deployment, auth, R2 storage, CVE scanning",
    "storage/datasette-sqlite-tools": "Datasette, Turso/libSQL, SQLPage — open-source SQLite tools with web UI, auto APIs, and enhanced features for exploring and managing databases",
    "monitoring/beszel": "Beszel — lightweight server monitoring with historical data, Docker stats, alerts, and OIDC/OAuth support",
